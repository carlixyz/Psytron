...
class VisualDialogManager : public Singleton<VisualDialogManager>
{
	friend class Singleton<VisualDialogManager>;
...
protected:
	struct ImageEase : public raylib::TextureUnmanaged
	{
		...
		void ActionScrollClampVertical()
		{
			Alpha = EaseCubicOut(CurrentTime, 0.0f, 1.0f, 2.0);
			Size.y = EaseLinearOut(CurrentTime, StartValue, EndValue - StartValue, TotalTime);
			Completed = Size.y > 0 && Graphics::Get().GetWindowArea().height + Size.y < Size.height ? false : true;
		}
		...
		void ActionScrollLoopVertical()
		{
			Alpha = EaseCubicOut(CurrentTime, 0.0f, 1.0f, 2.0);
			Size.y = EaseSineIn(CurrentTime, StartValue, EndValue - StartValue, TotalTime) * 0.5f;
		}
